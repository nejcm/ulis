...
- **Provenance Record**: Install-time state written by every build to `<source>/generated/.ulis-provenance.json`, naming the remote sources (if any) that contributed to each generated platform's output, keyed by platform. A later install that skips the rebuild reads it, scoped to the platforms it is installing, and refuses rather than deploy a tree it cannot re-preview; a local rebuild clears the record only for the platforms it targets, leaving other platforms' entries from a wider prior build in place. It is the state that closes the gap the Trust Gate alone leaves across two separate runs.
