...
- **Preset**: A reusable ULIS source tree resolved from user-global or bundled preset directories, or from a remote source. A preset can be layered before a base source, where the base source wins conflicts, or installed by itself as a preset-only source. For `permissions.yaml` list fields (`claude.allow`/`deny`/`ask`/`additionalDirectories`, `cursor.mcpAllowlist`/`terminalAllowlist`), winning a conflict means replacing the whole list, not appending to it: the highest layer to declare a given field wins it outright, including a layer that declares it empty; a layer that omits the field leaves the value from the layer below untouched. Within the surviving list, repeated entries from a single layer are deduplicated, preserving first occurrence.
