...
- **Cross-run bypass.** `ulis build --preset <url>` followed later by `ulis install --skip-rebuild` installed the remote preset's MCP servers, hooks, and raw config fragments with no prompt at all, because the second run had no way to know the tree it was about to install came from a remote source. Fixed: a build now records which platforms' generated output came from a remote source, and `--skip-rebuild` refuses if any platform it is about to install still carries that record. See **Behaviour changes** below — this is the fix behind two of the three breaking changes in this release.

- **The provenance record above could itself be defeated, two ways.** A record whose `remoteSources` field was an array (`[]`) or named an unknown or mis-cased platform key parsed as valid and was silently treated as "no remote source" — no file corruption needed, just a hand-edited or future-format record. And a symlinked platform output directory was invisible to the check deciding whether a local rebuild could safely discard a record it could not make sense of, so a narrow `ulis build --target <platform>` rebuild could discard a still-valid record while leaving a symlinked, remote-authored directory untouched on disk. Fixed: the record's shape is now strictly validated (an unrecognised key makes the whole record unreadable rather than being ignored), and symlinked platform directories are resolved and counted.
...
1. **A previously-approved `--skip-rebuild` may now refuse.** If you have run `ulis install --preset <url>` (or `--source <url>`) and approved its commands, that install now leaves a record behind naming the platforms it touched. Any _later_ `ulis install --skip-rebuild` against that same generated tree hard-fails with an error naming the recorded source — `-y`/`--yes` does **not** override this. It is deliberate: skipping the rebuild also skips the one thing that lets the gate re-verify what it is about to install.
