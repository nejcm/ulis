...
Requires Node 20.3+. The interactive dashboard (`ulis tui`) additionally requires [Bun](https://bun.sh), because its renderer runs through Bun's FFI — see [`ulis tui`](/CLI#ulis-tui). Every other command runs on Node.
